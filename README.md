...
## 三、运动恢复结构
作为多视图立体匹配的前置步骤，运动恢复结构（稀疏重建） 为输入图像进行相机参数估计和场景稀疏重建。 运动恢复结构的输出结果为输入图像对应的**相机位姿**以及**场景稀疏点云**。运动恢复结构可分为**增量式重建**和**全局式重建**，以增量式运动恢复结构为例，其重建流程如下图所示。
...
### 3.1 对应点搜索
它通常从特征提取和匹配开始，然后是几何验证来消除误匹配点对。（特征点提取-->特征点匹配-->外点剔除/误匹配剔除）

### 3.2 增量式重建

### 3.3 SfM拓展
<details>
  <summary>SfM拓展学习</summary>
  1. 增量式SfM发展史 Bundler --> Visual SfM --> COLMAP
  2. 全局式SfM相关资料 OpenMVG --> DAGSfM
  3. 超大规模SfM 权龙老师课题组学生的工作
  4. colmap 与深度学习结合的工作
  5. 纯深度学习做SfM的工作
  6. 重复纹理场景下的SfM
  7. 非刚体SfM
